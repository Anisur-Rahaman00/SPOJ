...
lls getSum(int pos, int n, lls sum, int f) {
   if (pos > n) return sum;
   ...
   for (int i = 0; i <= limit; i++) {
      if (f or i < limit) res += getSum(pos + 1, n, sum + i, 1);
...
void Run_Case() 
{
   ...
   while (cin >> a >> b and a != -1 and b != -1) {
      ...
      lls left = getSum(1, s.size(), 0, 0);

      for (int i = 0; i < t.size(); i++) arr[i + 1] = t[i] - '0';

      memset(dp, -1, sizeof(dp));
      lls right = getSum(1, t.size(), 0, 0);

      cout << right - left + getSum(a) << N;
   }
}
